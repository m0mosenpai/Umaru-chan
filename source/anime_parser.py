        ...
        def removeEpisodeNum(self):
                for i, string in enumerate(self.cleanedList):
                        ...
                        while tmpString.find(" - ") != -1:
                                idx = tmpString.find(" - ")
                                cutoff = idx
                                if tmpString[idx + 3:].isdigit():
                                        episodeNum = tmpString[idx + 3:]
                                        self.cleanedList[i] = string.replace(episodeNum, "").strip(" ")
                                        return episodeNum
                                elif tmpString[:idx].isdigit():
                                        episodeNum = tmpString[:idx]
                                        self.cleanedList[i] = string.replace(episodeNum, "").strip(" ")
                                        return episodeNum
                                for token in epTokens:
                                        idx = tmpString.find(token)
                                        if idx != -1 and lowerStr[idx + len(token)].isdigit():
                                                episodeNum = tmpString[idx + len(token):]
